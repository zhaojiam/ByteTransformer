...
int main(int argc, char *argv[]) {
  ...
  check_cuda_error(DPCT_CHECK_ERROR(stream = dev_ct1.create_queue()));
  dpct::queue_ptr cublas_handle;
  check_cuda_error(DPCT_CHECK_ERROR(cublas_handle = &dpct::get_in_order_queue()));
  check_cuda_error(DPCT_CHECK_ERROR(cublas_handle = stream));
  ...
  if (use_fp32) {
    ...
    sycl::half *attr_output_layernorm_gamma, *attr_output_layernorm_beta, *output_layernorm_gamma,
        *output_layernorm_beta;
    device_malloc(&attr_output_layernorm_gamma, hidden_dim);
    load_weights(&attr_output_layernorm_gamma, (char *)"attr_output_layernorm_gamma.in",
                 hidden_dim);
