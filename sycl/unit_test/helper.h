...
void device_malloc(T **ptr, int size) {
  *ptr = sycl::malloc_device<T>(size, dpct::get_in_order_queue());
}
...
void load_weights(T **ptr, char *file, int size) {
  ...
  dpct::get_in_order_queue().memcpy(*ptr, h_ptr, sizeof(T) * size).wait();
  free(h_ptr);
}
...
void result_check(const T *ptr, char *file, int size) {
  T *h_ptr = (T *)malloc(sizeof(T) * size);
  dpct::get_in_order_queue().memcpy(h_ptr, ptr, sizeof(T) * size).wait();
...
void print_vec(const T *data, const char *str, const int size) {
  ...
  dpct::get_in_order_queue().memcpy(tmp, data, sizeof(T) * size).wait();
