...
void softmax_kernel_warp_et(T *qk_buf, const T *atten_bias, const T *atten_mask,
                                       ...
                                       const sycl::nd_item<3> &item_ct1, uint8_t *dpct_local) {
  /*
  DPCT1098:335: The '*' expression is used instead of the __ldg call. These two expressions do not
  provide the exact same functionality. Check the generated code for potential precision and/or
  performance issues.
  */
  /*
  DPCT1064:336: Migrated __ldg call is used in a macro/template definition and may not be valid for
...
void softmax_kernel_warp_half2_et(sycl::half2 *qk_buf, const sycl::half2 *atten_bias,
                                  ...
                                  uint8_t *dpct_local) {
  /*
  DPCT1098:337: The '*' expression is used instead of the __ldg call. These two expressions do not
  provide the exact same functionality. Check the generated code for potential precision and/or
  performance issues.
  */
  /*
  DPCT1064:338: Migrated __ldg call is used in a macro/template definition and may not be valid for
  all macro/template uses. Adjust the code.
  */
  int word_id = word_idx[item_ct1.get_group(2)];
  int batch_id = word_id / seq_len;
...
void softmax_kernel_warp_half2_register(sycl::half2 *qk_buf, const sycl::half2 *atten_bias,
  ...
  for (int i = 0; i < count / 2; i++) {
    ...
    if (atten_bias)
      qk =
          /*
          DPCT1098:339: The '*' expression is used instead of the __ldg call. These two expressions
    ...
    /*
    DPCT1098:340: The '*' expression is used instead of the __ldg call. These two expressions do
...
void softmax_kernel_warp_half2_register_et(sycl::half2 *qk_buf, const sycl::half2 *atten_bias,
                                           ...
                                           int *word_idx, const sycl::nd_item<3> &item_ct1) {
  /*
  DPCT1098:341: The '*' expression is used instead of the __ldg call. These two expressions do not
  provide the exact same functionality. Check the generated code for potential precision and/or
  performance issues.
  */
  /*
  DPCT1064:344: Migrated __ldg call is used in a macro/template definition and may not be valid for
  all macro/template uses. Adjust the code.
  */
  int word_id = word_idx[item_ct1.get_group(2)];
  int batch_id = word_id / seq_len;
  ...
  for (int i = 0; i < count / 2; i++) {
    ...
    if (atten_bias)
      qk =
          /*
          DPCT1098:342: The '*' expression is used instead of the __ldg call. These two expressions
          do not provide the exact same functionality. Check the generated code for potential
          precision and/or performance issues.
          */
          qk + atten_bias[((head_id * seq_len + seq_id) * half2_seq_len) + col_id];
    /*
    DPCT1098:343: The '*' expression is used instead of the __ldg call. These two expressions do
    not provide the exact same functionality. Check the generated code for potential precision
    and/or performance issues.
    */
    sycl::half2 mask_val = atten_mask[((batch_id * seq_len + seq_id) * half2_seq_len) + col_id];
...
/*
DPCT1049:62: The work-group size passed to the SYCL kernel may exceed the limit. To get the device
limit, query info::device::max_work_group_size. Adjust the work-group size if needed.
*/
/*
DPCT1049:63: The work-group size passed to the SYCL kernel may exceed the limit. To get the device
limit, query info::device::max_work_group_size. Adjust the work-group size if needed.
*/
...
/*
DPCT1049:67: The work-group size passed to the SYCL kernel may exceed the limit. To get the device
limit, query info::device::max_work_group_size. Adjust the work-group size if needed.
*/
/*
DPCT1049:68: The work-group size passed to the SYCL kernel may exceed the limit. To get the device
limit, query info::device::max_work_group_size. Adjust the work-group size if needed.
*/
...
void softmax_kernelLauncher(T *qk_buf, const T *atten_bias, const T *atten_mask,
  ...
  /*
  DPCT1083:65: The size of local memory in the migrated code may be different from the original
  ...
  if ((seq_len & 0x1) == 0 && OpType == OperationType::HALF) {
    if (seq_len <= 1024) {
      ...
      if (shmem_size > 48 * 1024)
        /*
        DPCT1026:345: The call to cudaFuncSetAttribute was removed because SYCL currently does not
        support corresponding setting.
        */
        ;
      /*
      DPCT1049:64: The work-group size passed to the SYCL kernel may exceed the limit. To get the
    ...
    if (shmem_size > 48 * 1024)
      /*
      DPCT1026:346: The call to cudaFuncSetAttribute was removed because SYCL currently does not
      support corresponding setting.
      */
      ;
    /*
    DPCT1049:66: The work-group size passed to the SYCL kernel may exceed the limit. To get the
    device limit, query info::device::max_work_group_size. Adjust the work-group size if needed.
    */
    {
      dpct::has_capability_or_fail(stream->get_device(), {sycl::aspect::fp16});
...
void softmax_et_kernelLauncher(T *qk_buf, const T *atten_bias, const T *atten_mask,
  ...
  /*
  DPCT1083:70: The size of local memory in the migrated code may be different from the original
  code. Check that the allocated memory size in the migrated code is correct.
  */
  const int shmem_size = head_num * seq_len * sizeof(float);
  if (shmem_size > 64 * 1024)
  ...
  if ((seq_len & 0x1) == 0 && OpType == OperationType::HALF) {
    if (seq_len <= 1024) {
      ...
      if (shmem_size > 48 * 1024)
        /*
        DPCT1026:347: The call to cudaFuncSetAttribute was removed because SYCL currently does not
        support corresponding setting.
        */
        ;
      /*
      DPCT1049:69: The work-group size passed to the SYCL kernel may exceed the limit. To get the
      device limit, query info::device::max_work_group_size. Adjust the work-group size if needed.
      */
      {
        dpct::has_capability_or_fail(stream->get_device(), {sycl::aspect::fp16});
    ...
    if (shmem_size > 48 * 1024)
      /*
      DPCT1026:348: The call to cudaFuncSetAttribute was removed because SYCL currently does not
      support corresponding setting.
      */
      ;
    /*
    DPCT1049:71: The work-group size passed to the SYCL kernel may exceed the limit. To get the
    device limit, query info::device::max_work_group_size. Adjust the work-group size if needed.
    */
    {
      dpct::has_capability_or_fail(stream->get_device(), {sycl::aspect::fp16});
