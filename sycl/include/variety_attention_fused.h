...
void variety_wmma_attention_kernel(const sycl::half2 *query, const sycl::half2 *key,
  ...
  for (int seq_id = warpId; seq_id < seq_len; seq_id += warpNums) {
    ...
    /*
    DPCT1098:349: The '*' expression is used instead of the __ldg call. These two expressions do
    ...
    *(sycl::half2 *)(*s_query + offset) = query[pos];
    /*
    DPCT1098:350: The '*' expression is used instead of the __ldg call. These two expressions do
    not provide the exact same functionality. Check the generated code for potential precision
    and/or performance issues.
    */
  ...
  */
  ...
  if (warpId < from_size * to_size) {
    /*
    DPCT1082:73: Migration of nvcuda::wmma::fragment<wmma::matrix_a, 16, 16, 16, half,
    wmma::row_major> type is not supported.
    */
    /*
    DPCT1082:74: Migration of nvcuda::wmma::matrix_a type is not supported.
    */
    /*
    DPCT1082:75: Migration of nvcuda::wmma::row_major type is not supported.
    */
...
#pragma unroll
    for (int k = 0; k < 4; k++) {
      /*
      DPCT1007:94: Migration of nvcuda::wmma::load_matrix_sync is not supported.
      */
      wmma::load_matrix_sync(Q_mat, s_query[warp_from_offset] + k * WMMA_K,
                             size_per_head + SKEW_HALF);
      /*
      DPCT1007:95: Migration of nvcuda::wmma::load_matrix_sync is not supported.
      */
      wmma::load_matrix_sync(K_mat, s_kv[warp_to_offset] + k * WMMA_K, size_per_head + SKEW_HALF);
      /*
      DPCT1007:82: Migration of nvcuda::wmma::mma_sync is not supported.
      */
      wmma::mma_sync(QK_mat, Q_mat, K_mat, QK_mat);
    }
    /*
    DPCT1007:93: Migration of nvcuda::wmma::store_matrix_sync is not supported.
    */
...
#pragma unroll
    for (int i = 0; i < n; i++) {
      ...
      if (to_id[i] < seq_len) {
        ...
        float mask =
            /*
            DPCT1098:356: The '*' expression is used instead of the __ldg call. These two
...
#pragma unroll
    ...
    /*
    DPCT1098:351: The '*' expression is used instead of the __ldg call. These two expressions do
    not provide the exact same functionality. Check the generated code for potential precision
    and/or performance issues.
    */
  ...
  */
  ...
  if (warpId < (from_size << 2)) {
    /*
    DPCT1082:83: Migration of nvcuda::wmma::fragment<wmma::matrix_a, 16, 16, 16, half,
    wmma::row_major> type is not supported.
    */
    /*
    DPCT1082:84: Migration of nvcuda::wmma::matrix_a type is not supported.
    */
    /*
    DPCT1082:85: Migration of nvcuda::wmma::row_major type is not supported.
    */
    wmma::fragment < wmma::matrix_a, WMMA_M, WMMA_N, WMMA_K,
...
#pragma unroll
    for (int k = 0; k < to_size; k++) {
      /*
      DPCT1007:97: Migration of nvcuda::wmma::load_matrix_sync is not supported.
      */
      wmma::load_matrix_sync(Logits_mat, s_logits[warp_from_offset] + k * WMMA_K,
                             max_seq_len + SKEW_HALF);
      /*
      DPCT1007:98: Migration of nvcuda::wmma::load_matrix_sync is not supported.
      */
      wmma::load_matrix_sync(V_mat, s_kv[k * WMMA_K] + warp_to_offset, size_per_head + SKEW_HALF);
      /*
      DPCT1007:92: Migration of nvcuda::wmma::mma_sync is not supported.
      */
      wmma::mma_sync(QKV_mat, Logits_mat, V_mat, QKV_mat);
    }
    /*
    DPCT1007:96: Migration of nvcuda::wmma::store_matrix_sync is not supported.
    */
...
void variety_wmma_attention_rm_kernel(
  ...
  const int thread_offset = item_ct1.get_group(2) * (size_per_head / 2) + warp_tid;
  /*
  DPCT1098:357: The '*' expression is used instead of the __ldg call. These two expressions do not
  ...
  */
  ...
  /*
  DPCT1098:358: The '*' expression is used instead of the __ldg call. These two expressions do not
  provide the exact same functionality. Check the generated code for potential precision and/or
  performance issues.
  */
  ...
  for (int seq_id = warpId; seq_id < batch_seq_len; seq_id += warpNums) {
    ...
    int offset = seq_id * (size_per_head + SKEW_HALF) + (warp_tid << 1);
    /*
    DPCT1098:359: The '*' expression is used instead of the __ldg call. These two expressions do
    not provide the exact same functionality. Check the generated code for potential precision
    and/or performance issues.
    */
    *(sycl::half2 *)(*s_query + offset) = query[pos];
    /*
    DPCT1098:360: The '*' expression is used instead of the __ldg call. These two expressions do
    not provide the exact same functionality. Check the generated code for potential precision
    and/or performance issues.
    */
    *(sycl::half2 *)(*s_kv + offset) = key[pos];
  ...
  */
  ...
  if (warpId < from_size * to_size) {
    /*
    DPCT1082:100: Migration of nvcuda::wmma::fragment<wmma::matrix_a, 16, 16, 16, half,
    wmma::row_major> type is not supported.
    */
    /*
    DPCT1082:101: Migration of nvcuda::wmma::matrix_a type is not supported.
    */
    /*
    DPCT1082:102: Migration of nvcuda::wmma::row_major type is not supported.
    */
    wmma::fragment < wmma::matrix_a, WMMA_M, WMMA_N, WMMA_K,
        wmma::fragment<wmma::matrix_a, WMMA_M, WMMA_N, WMMA_K, sycl::half, wmma::row_major> _N,
        WMMA_K,
...
#pragma unroll
    for (int k = 0; k < 4; k++) {
      /*
      DPCT1007:121: Migration of nvcuda::wmma::load_matrix_sync is not supported.
      */
      wmma::load_matrix_sync(Q_mat, s_query[warp_from_offset] + k * WMMA_K,
                             size_per_head + SKEW_HALF);
      /*
      DPCT1007:122: Migration of nvcuda::wmma::load_matrix_sync is not supported.
      */
      wmma::load_matrix_sync(K_mat, s_kv[warp_to_offset] + k * WMMA_K, size_per_head + SKEW_HALF);
      /*
      DPCT1007:109: Migration of nvcuda::wmma::mma_sync is not supported.
      */
      wmma::mma_sync(QK_mat, Q_mat, K_mat, QK_mat);
    }
    /*
    DPCT1007:120: Migration of nvcuda::wmma::store_matrix_sync is not supported.
    */
    wmma::store_matrix_sync(s_logits[warp_from_offset] + warp_to_offset, QK_mat,
                            max_seq_len + SKEW_HALF, wmma::mem_row_major);
  }
...
#pragma unroll
    ...
    int pos = (gemm_offset + from_id) * (size_per_head / 2) + warp_tid;
    /*
    DPCT1098:361: The '*' expression is used instead of the __ldg call. These two expressions do
    not provide the exact same functionality. Check the generated code for potential precision
    and/or performance issues.
    */
    ((sycl::half2 *)(s_kv[from_id]))[warp_tid] = value[pos];
  ...
  */
  ...
  if (warpId < (from_size << 2)) {
    /*
    DPCT1082:110: Migration of nvcuda::wmma::fragment<wmma::matrix_a, 16, 16, 16, half,
    wmma::row_major> type is not supported.
    */
    /*
    DPCT1082:111: Migration of nvcuda::wmma::matrix_a type is not supported.
    */
    /*
    DPCT1082:112: Migration of nvcuda::wmma::row_major type is not supported.
    */
    wmma::fragment < wmma::matrix_a, WMMA_M, WMMA_N, WMMA_K,
        wmma::fragment<wmma::matrix_a, WMMA_M, WMMA_N, WMMA_K, sycl::half, wmma::row_major> WMMA_N,
        WMMA_K,
...
#pragma unroll
    for (int k = 0; k < to_size; k++) {
      /*
      DPCT1007:124: Migration of nvcuda::wmma::load_matrix_sync is not supported.
      */
      wmma::load_matrix_sync(Logits_mat, s_logits[warp_from_offset] + k * WMMA_K,
                             max_seq_len + SKEW_HALF);
      /*
      DPCT1007:125: Migration of nvcuda::wmma::load_matrix_sync is not supported.
      */
      wmma::load_matrix_sync(V_mat, s_kv[k * WMMA_K] + warp_to_offset, size_per_head + SKEW_HALF);
      /*
      DPCT1007:119: Migration of nvcuda::wmma::mma_sync is not supported.
      */
      wmma::mma_sync(QKV_mat, Logits_mat, V_mat, QKV_mat);
    }
    /*
    DPCT1007:123: Migration of nvcuda::wmma::store_matrix_sync is not supported.
    */
    wmma::store_matrix_sync(s_query[warp_from_offset] + warp_to_offset, QKV_mat,
                            size_per_head + SKEW_HALF, wmma::mem_row_major);
  }
...
void variety_wmma_attention_long_kernel(const sycl::half2 *query, const sycl::half2 *key,
  ...
  for (int seq_id = warpId; seq_id < block_seq_len; seq_id += warpNums) {
    ...
    int offset = seq_id * (size_per_head + SKEW_HALF) + (warp_tid << 1);
    /*
    DPCT1098:366: The '*' expression is used instead of the __ldg call. These two expressions do
    not provide the exact same functionality. Check the generated code for potential precision
    and/or performance issues.
    */
    *(sycl::half2 *)(*s_query + offset) = query[pos];
  ...
  for (int seq_id = warpId; seq_id < seq_len; seq_id += warpNums) {
    ...
    int offset = seq_id * (size_per_head + SKEW_HALF) + (warp_tid << 1);
    /*
    DPCT1098:367: The '*' expression is used instead of the __ldg call. These two expressions do
    not provide the exact same functionality. Check the generated code for potential precision
    and/or performance issues.
    */
    *(sycl::half2 *)(*s_kv + offset) = key[pos];
  ...
  */
  ...
  if (warpId < from_size * to_size) {
    /*
    DPCT1082:127: Migration of nvcuda::wmma::fragment<wmma::matrix_a, 16, 16, 16, half,
    wmma::row_major> type is not supported.
    */
    /*
    DPCT1082:128: Migration of nvcuda::wmma::matrix_a type is not supported.
    */
    /*
    DPCT1082:129: Migration of nvcuda::wmma::row_major type is not supported.
    */
    wmma::fragment < wmma::matrix_a, WMMA_M, WMMA_N, WMMA_K,
        wmma::fragment<wmma::matrix_a, WMMA_M, WMMA_N, WMMA_K, sycl::half, wmma::row_major> _N,
        WMMA_K,
...
#pragma unroll
    for (int k = 0; k < 4; k++) {
      /*
      DPCT1007:148: Migration of nvcuda::wmma::load_matrix_sync is not supported.
      */
      wmma::load_matrix_sync(Q_mat, s_query[warp_from_offset] + k * WMMA_K,
                             size_per_head + SKEW_HALF);
      /*
      DPCT1007:149: Migration of nvcuda::wmma::load_matrix_sync is not supported.
      */
      wmma::load_matrix_sync(K_mat, s_kv[warp_to_offset] + k * WMMA_K, size_per_head + SKEW_HALF);
      /*
      DPCT1007:136: Migration of nvcuda::wmma::mma_sync is not supported.
      */
      wmma::mma_sync(QK_mat, Q_mat, K_mat, QK_mat);
    }
    /*
    DPCT1007:147: Migration of nvcuda::wmma::store_matrix_sync is not supported.
    */
    wmma::store_matrix_sync(s_logits[warp_from_offset] + warp_to_offset, QK_mat,
                            max_seq_len + SKEW_HALF, wmma::mem_row_major);
  }
  /*
  DPCT1065:370: Consider replacing sycl::nd_item::barrier() with
  sycl::nd_item::barrier(sycl::access::fence_space::local_space) for better performance if there is
  no access to global memory.
  */
  item_ct1.barrier();
...
#pragma unroll
    for (int i = 0; i < n; i++) {
      ...
      if (to_id[i] < seq_len) {
        ...
        /*
        DPCT1098:373: The '*' expression is used instead of the __ldg call. These two expressions
...
#pragma unroll
  ...
  for (int seq_id = warpId; seq_id < seq_len; seq_id += warpNums) {
    int pos = (gemm_offset + seq_id) * (size_per_head / 2) + warp_tid;
    /*
    DPCT1098:368: The '*' expression is used instead of the __ldg call. These two expressions do
    not provide the exact same functionality. Check the generated code for potential precision
    and/or performance issues.
    */
  ...
  */
  ...
  if (warpId < (from_size << 2)) {
    /*
    DPCT1082:137: Migration of nvcuda::wmma::fragment<wmma::matrix_a, 16, 16, 16, half,
    wmma::row_major> type is not supported.
    */
    /*
    DPCT1082:138: Migration of nvcuda::wmma::matrix_a type is not supported.
    */
    /*
    DPCT1082:139: Migration of nvcuda::wmma::row_major type is not supported.
    */
    wmma::fragment < wmma::matrix_a, WMMA_M, WMMA_N, WMMA_K,
        wmma::fragment<wmma::matrix_a, WMMA_M, WMMA_N, WMMA_K, sycl::half, wmma::row_major> WMMA_N,
        WMMA_K,
...
#pragma unroll
    for (int k = 0; k < to_size; k++) {
      /*
      DPCT1007:151: Migration of nvcuda::wmma::load_matrix_sync is not supported.
      */
      wmma::load_matrix_sync(Logits_mat, s_logits[warp_from_offset] + k * WMMA_K,
                             max_seq_len + SKEW_HALF);
      /*
      DPCT1007:152: Migration of nvcuda::wmma::load_matrix_sync is not supported.
      */
      wmma::load_matrix_sync(V_mat, s_kv[k * WMMA_K] + warp_to_offset, size_per_head + SKEW_HALF);
      /*
      DPCT1007:146: Migration of nvcuda::wmma::mma_sync is not supported.
      */
      wmma::mma_sync(QKV_mat, Logits_mat, V_mat, QKV_mat);
    }
    /*
    DPCT1007:150: Migration of nvcuda::wmma::store_matrix_sync is not supported.
    */
    wmma::store_matrix_sync(s_query[warp_from_offset] + warp_to_offset, QKV_mat,
                            size_per_head + SKEW_HALF, wmma::mem_row_major);
  }
...
template <const int max_seq_len, const int size_per_head, const int split_seq_len>
/*
DPCT1110:153: The total declared local variable size in device function
...
void variety_wmma_attention_long_rm_kernel(
  ...
  /*
  DPCT1098:374: The '*' expression is used instead of the __ldg call. These two expressions do not
  provide the exact same functionality. Check the generated code for potential precision and/or
  performance issues.
  */
  const int batch_seq_offset = batch_idx[item_ct1.get_group(0)];
  /*
  DPCT1098:375: The '*' expression is used instead of the __ldg call. These two expressions do not
  provide the exact same functionality. Check the generated code for potential precision and/or
  performance issues.
  */
  ...
  for (int seq_id = warpId; seq_id < block_seq_len; seq_id += warpNums) {
    ...
    int offset = seq_id * (size_per_head + SKEW_HALF) + (warp_tid << 1);
    /*
    DPCT1098:376: The '*' expression is used instead of the __ldg call. These two expressions do
    not provide the exact same functionality. Check the generated code for potential precision
    and/or performance issues.
    */
    *(sycl::half2 *)(*s_query + offset) = query[pos];
  ...
  for (int seq_id = warpId; seq_id < batch_seq_len; seq_id += warpNums) {
    ...
    int offset = seq_id * (size_per_head + SKEW_HALF) + (warp_tid << 1);
    /*
    DPCT1098:377: The '*' expression is used instead of the __ldg call. These two expressions do
    not provide the exact same functionality. Check the generated code for potential precision
    and/or performance issues.
    */
    *(sycl::half2 *)(*s_kv + offset) = key[pos];
  ...
  */
  ...
  if (warpId < from_size * to_size) {
    /*
    DPCT1082:154: Migration of nvcuda::wmma::fragment<wmma::matrix_a, 16, 16, 16, half,
    wmma::row_major> type is not supported.
    */
    /*
    DPCT1082:155: Migration of nvcuda::wmma::matrix_a type is not supported.
    */
    /*
    DPCT1082:156: Migration of nvcuda::wmma::row_major type is not supported.
    */
    wmma::fragment < wmma::matrix_a, WMMA_M, WMMA_N, WMMA_K,
        wmma::fragment<wmma::matrix_a, WMMA_M, WMMA_N, WMMA_K, sycl::half, wmma::row_major> _N,
        WMMA_K,
...
#pragma unroll
      for (int k = 0; k < 4; k++) {
        /*
        DPCT1007:175: Migration of nvcuda::wmma::load_matrix_sync is not supported.
        */
        wmma::load_matrix_sync(Q_mat, s_query[warp_from_offset] + k * WMMA_K,
                               size_per_head + SKEW_HALF);
        /*
        DPCT1007:176: Migration of nvcuda::wmma::load_matrix_sync is not supported.
        */
        wmma::load_matrix_sync(K_mat, s_kv[warp_to_offset] + k * WMMA_K,
                               size_per_head + SKEW_HALF);
        /*
        DPCT1007:163: Migration of nvcuda::wmma::mma_sync is not supported.
        */
        wmma::mma_sync(QK_mat, Q_mat, K_mat, QK_mat);
      }
      /*
      DPCT1007:174: Migration of nvcuda::wmma::store_matrix_sync is not supported.
      */
      wmma::store_matrix_sync(s_logits[warp_from_offset] + warp_to_offset, QK_mat,
                              max_seq_len + SKEW_HALF, wmma::mem_row_major);
    }
...
#pragma unroll
  ...
  for (int seq_id = warpId; seq_id < batch_seq_len; seq_id += warpNums) {
    int pos = (gemm_offset + seq_id) * (size_per_head / 2) + warp_tid;
    /*
    DPCT1098:378: The '*' expression is used instead of the __ldg call. These two expressions do
    not provide the exact same functionality. Check the generated code for potential precision
    and/or performance issues.
    */
    ((sycl::half2 *)(s_kv[seq_id]))[warp_tid] = value[pos];
  ...
  */
  ...
  if (warpId < (from_size << 2)) {
    /*
    DPCT1082:164: Migration of nvcuda::wmma::fragment<wmma::matrix_a, 16, 16, 16, half,
    wmma::row_major> type is not supported.
    */
    /*
    DPCT1082:165: Migration of nvcuda::wmma::matrix_a type is not supported.
    */
    /*
    DPCT1082:166: Migration of nvcuda::wmma::row_major type is not supported.
    */
    wmma::fragment < wmma::matrix_a, WMMA_M, WMMA_N, WMMA_K,
        wmma::fragment<wmma::matrix_a, WMMA_M, WMMA_N, WMMA_K, sycl::half, wmma::row_major> WMMA_N,
        WMMA_K,
...
#pragma unroll
    for (int k = 0; k < (batch_seq_len_pad / 16); k++) {
      /*
      DPCT1007:178: Migration of nvcuda::wmma::load_matrix_sync is not supported.
      */
      wmma::load_matrix_sync(Logits_mat, s_logits[warp_from_offset] + k * WMMA_K,
                             max_seq_len + SKEW_HALF);
      /*
      DPCT1007:179: Migration of nvcuda::wmma::load_matrix_sync is not supported.
      */
      wmma::load_matrix_sync(V_mat, s_kv[k * WMMA_K] + warp_to_offset, size_per_head + SKEW_HALF);
      /*
      DPCT1007:173: Migration of nvcuda::wmma::mma_sync is not supported.
      */
      wmma::mma_sync(QKV_mat, Logits_mat, V_mat, QKV_mat);
    }
    /*
    DPCT1007:177: Migration of nvcuda::wmma::store_matrix_sync is not supported.
    */
    wmma::store_matrix_sync(s_query[warp_from_offset] + warp_to_offset, QKV_mat,
                            size_per_head + SKEW_HALF, wmma::mem_row_major);
  }
...
/*
DPCT1049:180: The work-group size passed to the SYCL kernel may exceed the limit. To get the device
...
/*
DPCT1049:181: The work-group size passed to the SYCL kernel may exceed the limit. To get the device
limit, query info::device::max_work_group_size. Adjust the work-group size if needed.
*/
...
/*
DPCT1049:182: The work-group size passed to the SYCL kernel may exceed the limit. To get the device
limit, query info::device::max_work_group_size. Adjust the work-group size if needed.
*/
/*
DPCT1026:383: The call to cudaFuncSetAttribute was removed because SYCL currently does not support
...
/*
DPCT1049:183: The work-group size passed to the SYCL kernel may exceed the limit. To get the device
limit, query info::device::max_work_group_size. Adjust the work-group size if needed.
*/
/*
DPCT1026:384: The call to cudaFuncSetAttribute was removed because SYCL currently does not support
corresponding setting.
*/
