...
__inline__ T act_fun(T val) {
  if (act == ActType::Relu)
    ...
    /*
    DPCT1064:550: Migrated __expf call is used in a macro/template definition and may not be valid
    for all macro/template uses. Adjust the code.
    */
    return sycl::log(sycl::native::exp((float)val) + 1.0f);
  else if (act == ActType::Sigmoid)
    /*
    DPCT1064:551: Migrated __expf call is used in a macro/template definition and may not be valid
    for all macro/template uses. Adjust the code.
    */
...
static const char *_cudaGetErrorEnum(dpct::err0 error) {
  /*
  DPCT1009:303: SYCL uses exceptions to report errors and does not use the error codes. The
...
void check(T result, char const *const func, const char *const file, int const line) {
  /*
  DPCT1000:553: Error handling if-stmt was detected but could not be rewritten.
  */
  if (result)
    /*
    DPCT1001:552: The statement could not be removed.
    */
