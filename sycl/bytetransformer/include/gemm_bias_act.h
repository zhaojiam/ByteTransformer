...
__dpct_inline__ float copysignf_pos(float a, float b) {
  return sycl::bit_cast<float, int>(sycl::bit_cast<int>(a) |
                                    (sycl::bit_cast<int>(b) & 0x80000000));
}

// Compute hyperbolic tangent for < sm_75. maxulperr = 108.82848, maxrelerr = 9.3450e-6
__dpct_inline__ float __tanhf(float a) {
  const float L2E = 1.442695041f;
  float e, r, s, t, d;
  s = sycl::fabs(a);
  t = -L2E * 2.0f * s;
  e = sycl::pow<float>(2, t);
  d = e + 1.0f;
  /*
  DPCT1053:228: Migration of device assembly code is not supported.
  */
...
__dpct_inline__ float fast_tanh(float lhs) {
#if defined(DPCT_COMPATIBILITY_TEMP) && (DPCT_COMPATIBILITY_TEMP >= 750)
  float ret;
  ret = sycl::tanh<float>(lhs);
  return ret;
#else
  // return ::tanhf(lhs);
  return __tanhf(lhs);
...
template <typename T>
struct GELUFAST {
  T operator()(T const &scalar_t) const {
...
__inline__ float tanht(float lhs) {
#if defined(DPCT_COMPATIBILITY_TEMP) && (DPCT_COMPATIBILITY_TEMP >= 750)
  float ret;
  ret = sycl::tanh<float>(lhs);
  return ret;
...
template <>
__inline__ sycl::half2 gelu(sycl::half2 val) {
  sycl::half2 val_pow3 = val * val * val;
  sycl::float2 tmp_pow = val_pow3.convert<float, sycl::rounding_mode::automatic>();
  sycl::float2 tmp = val.convert<float, sycl::rounding_mode::automatic>();
...
#endif
  return val * tmp.convert<sycl::half, sycl::rounding_mode::rte>();
}
...
template <typename T>
void add_bias_gelu(T *output, const T *bias, const int M, const int N,
                   const sycl::nd_item<3> &item_ct1);

template <typename T>
void gemm_bias_gelu(const T *A_, const T *B_, T *C_, const T *bias_, int m_, int k_, int n_,
                    dpct::queue_ptr stream, dpct::queue_ptr cublas_handle, int cublasAlgo,
                    int arch);
