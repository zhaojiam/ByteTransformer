...
__inline__ T warpPrefixSum(int id, T count, const sycl::nd_item<3> &item_ct1) {
  for (int i = 1; i < 32; i <<= 1) {
    /*
    DPCT1121:502: Make sure that the "count" which is used in the SYCL group function/algorithm is
    initialized.
    */
    /*
    DPCT1096:839: The right-most dimension of the work-group used in the SYCL kernel that calls
  ...
  */
  ...
  if (warp_id == 0) {
    ...
    for (int i = warp_tid; i < ((batch_size + 31) / 32) * 32; i += 32) {
      ...
      /*
      DPCT1096:838: The right-most dimension of the work-group used in the SYCL kernel that calls
...
void build_sequence_length_padding_offset_kernelLauncher(const T *atten_mask, int *batch_idx,
  ...
  if (block[2] > 1024)
    block[2] = 1024;
  /*
  DPCT1049:414: The work-group size passed to the SYCL kernel may exceed the limit. To get the
  ...
  */
  {
    ...
    stream->submit([&](sycl::handler &cgh) {
      /*
      DPCT1083:837: The size of local memory in the migrated code may be different from the
...
void compress_bert_input(const T *from_tensor, T *to_tensor, const int *batch_idx,
  ...
  /*
  DPCT1098:738: The '*' expression is used instead of the __ldg call. These two expressions do not
  provide the exact same functionality. Check the generated code for potential precision and/or
  performance issues.
  */
  /*
  DPCT1064:834: Migrated __ldg call is used in a macro/template definition and may not be valid for
...
void compressBertInput_kernelLauncher(const T *from_tensor, T *to_tensor, int *batch_idx,
  ...
  /*
  DPCT1049:415: The work-group size passed to the SYCL kernel may exceed the limit. To get the
  device limit, query info::device::max_work_group_size. Adjust the work-group size if needed.
  */
  {
    dpct::has_capability_or_fail(stream->get_device(), {sycl::aspect::fp16});
