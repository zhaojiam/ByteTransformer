...
void CutlassAttention<OpType>::infer_impl(AttentionInferParam *infer_param) {
  ...
  if constexpr (true) {
    ...
    if (this->is_remove_padding_) {
      /*
      DPCT1049:388: The work-group size passed to the SYCL kernel may exceed the limit. To get the
      ...
      */
      ...
      /*
      DPCT1049:389: The work-group size passed to the SYCL kernel may exceed the limit. To get the
      device limit, query info::device::max_work_group_size. Adjust the work-group size if needed.
      */
      dpct::has_capability_or_fail(stream->get_device(), {sycl::aspect::fp16});
