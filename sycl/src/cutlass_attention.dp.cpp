...
void gemm0_and_softmax_reduce_kernel_launcher(DataType *query, DataType *key, DataType *atten_mask,
                                              ...
                                              const int sm_count, dpct::queue_ptr stream) {
...
void CutlassAttention<OpType>::infer_impl(AttentionInferParam *infer_param) {
  ...
  if constexpr (true) {
    ...
    block[2] = head_num * size_per_head_half;
    if (this->is_remove_padding_) {
      /*
      DPCT1049:190: The work-group size passed to the SYCL kernel may exceed the limit. To get the
      ...
      */
      ...
      stream->submit([&](sycl::handler &cgh) {
        ...
        auto this_param__attr_bias_QKV_ct1 = this->param_.attr_bias_QKV;

        cgh.parallel_for(sycl::nd_range<3>(grid * block, block), [=](sycl::nd_item<3> item_ct1) {
          add_QKV_bias_padding(infer_param_qkv_ct0, this_param__attr_bias_QKV_ct1, query, key,
                               value, batch_size, seq_len, head_num, size_per_head_half,
                               is_roformer, et_param.batch_idx, et_param.word_idx, item_ct1);
        });
      });
    } else {
      /*
      DPCT1049:191: The work-group size passed to the SYCL kernel may exceed the limit. To get the
      device limit, query info::device::max_work_group_size. Adjust the work-group size if needed.
      */
      dpct::has_capability_or_fail(stream->get_device(), {sycl::aspect::fp16});
      ...
      stream->submit([&](sycl::handler &cgh) {
        ...
        cgh.parallel_for(sycl::nd_range<3>(grid * block, block), [=](sycl::nd_item<3> item_ct1) {
          add_QKV_bias(infer_param_qkv_ct0, this_param__attr_bias_QKV_ct1, query, key, value,
                       batch_size, seq_len, head_num, size_per_head_half, is_roformer, item_ct1);
        });
      });
    }
  }
...
void CutlassAttention<OpType>::infer(AttentionInferParam infer_param) {
  ...
  static_assert((DPCT_COMPAT_RT_MAJOR_VERSION * 100 + DPCT_COMPAT_RT_MINOR_VERSION) >= 1106,
