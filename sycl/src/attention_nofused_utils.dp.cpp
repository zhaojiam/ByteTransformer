...
void add_QKV_bias<float>(const float *QKV, const float *bias_QKV, float *q_buf, float *k_buf,
  ...
  /*
  DPCT1098:554: The '*' expression is used instead of the __ldg call. These two expressions do not
  ...
  */
  ...
  sycl::float2 k_value = ((sycl::float2 *)QKV)[src_id + item_ct1.get_local_range(2)],
               /*
               DPCT1098:555: The '*' expression is used instead of the __ldg call. These two
  ...
  sycl::float2 v_value = ((sycl::float2 *)QKV)[src_id + item_ct1.get_local_range(2) * 2],
               /*
               DPCT1098:556: The '*' expression is used instead of the __ldg call. These two
               expressions do not provide the exact same functionality. Check the generated code
               for potential precision and/or performance issues.
               */
      v_bias =
...
void add_QKV_bias<sycl::half>(const sycl::half *QKV, const sycl::half *bias_QKV, sycl::half *q_buf,
  ...
  sycl::half2 q_value =
      /*
      DPCT1098:557: The '*' expression is used instead of the __ldg call. These two expressions do
      not provide the exact same functionality. Check the generated code for potential precision
      and/or performance issues.
      */
      /*
      DPCT1064:558: Migrated __ldg call is used in a macro/template definition and may not be valid
...
void add_QKV_bias_padding<float>(const float *QKV, const float *bias_QKV, float *q_buf,
  ...
  int trt_id = ((head_id * batch_size + batch_id) * seq_len + seq_id) * half_size_per_head + id;
  /*
  DPCT1098:563: The '*' expression is used instead of the __ldg call. These two expressions do not
  provide the exact same functionality. Check the generated code for potential precision and/or
  performance issues.
  */
  const int batch_offset = batch_idx[item_ct1.get_group(1)];
  /*
  DPCT1098:564: The '*' expression is used instead of the __ldg call. These two expressions do not
  provide the exact same functionality. Check the generated code for potential precision and/or
  performance issues.
  */
  ...
  if (seq_id < batch_seq_len) {
    ...
    /*
    DPCT1098:565: The '*' expression is used instead of the __ldg call. These two expressions do
    not provide the exact same functionality. Check the generated code for potential precision
    and/or performance issues.
    */
    sycl::float2 q_value = ((sycl::float2 *)QKV)[src_id],
    ...
    sycl::float2 k_value = ((sycl::float2 *)QKV)[src_id + item_ct1.get_local_range(2)],
                 /*
                 DPCT1098:566: The '*' expression is used instead of the __ldg call. These two
                 expressions do not provide the exact same functionality. Check the generated code
                 for potential precision and/or performance issues.
                 */
        k_bias =
    ...
    sycl::float2 v_value = ((sycl::float2 *)QKV)[src_id + item_ct1.get_local_range(2) * 2],
                 /*
                 DPCT1098:567: The '*' expression is used instead of the __ldg call. These two
                 expressions do not provide the exact same functionality. Check the generated code
                 for potential precision and/or performance issues.
                 */
        v_bias =
...
void add_QKV_bias_padding<sycl::half>(const sycl::half *QKV, const sycl::half *bias_QKV,
  ...
  int trt_id = ((head_id * batch_size + batch_id) * seq_len + seq_id) * half_size_per_head + id;
  /*
  DPCT1098:568: The '*' expression is used instead of the __ldg call. These two expressions do not
  provide the exact same functionality. Check the generated code for potential precision and/or
  performance issues.
  */
  const int batch_offset = batch_idx[item_ct1.get_group(1)];
  /*
  DPCT1098:569: The '*' expression is used instead of the __ldg call. These two expressions do not
  provide the exact same functionality. Check the generated code for potential precision and/or
  performance issues.
  */
  const int batch_seq_len = batch_idx[item_ct1.get_group(1) + 1] - batch_offset;
  if (seq_id < batch_seq_len) {
    ...
    sycl::half2 q_value =
        /*
        DPCT1098:570: The '*' expression is used instead of the __ldg call. These two expressions
        do not provide the exact same functionality. Check the generated code for potential
        precision and/or performance issues.
        */
        /*
        DPCT1064:571: Migrated __ldg call is used in a macro/template definition and may not be
